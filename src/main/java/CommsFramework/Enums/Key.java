package CommsFramework.Enums;

public enum Key {
    pathNorth, pathWest, pathEast, pathSouth, action, hpPotionAvailable, manaPotionAvailable, status, visited, playerHp,
    playerMana, playerDamage, playerSkillDamage, playerIntelligence, playerVitality, playerHpPotionsCount,
    playerManaPotionsCount, playerSkillCost, playerManaLimit, playerHpLimit, loot, enemyName, enemyAlive,
    enemyAttacked, playerAttacked, playerUsedSkill, playerAlive, playerSkillAvailable
}
